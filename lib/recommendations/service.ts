...
async function similarProductIds(anchorId: string, exclude: Set<string>, limit: number): Promise<string[]> {
...
// Curated `product_accessories` always outranks rule-generated complements —
// curation is the merchant's override layer on top of the algorithm.
async function crossSellProductIds(anchorIds: string[], exclude: Set<string>, limit: number): Promise<string[]> {
  const curatedRes = await supabaseAdminV2
    .from("product_accessories")
    .select("product_id, accessory_product_id, sort_order")
    .in("product_id", anchorIds)
    .order("sort_order");
  const curated = ((curatedRes.data ?? []) as { accessory_product_id: string }[]).map((r) => r.accessory_product_id);
  ...
  if (ordered.length < limit) {
    const complementRes = await supabaseAdminV2
      .from("product_recommendations")
      .select("recommended_product_id, rank")
      .in("product_id", anchorIds)
      .eq("kind", "complement")
      .order("rank");
    for (const row of (complementRes.data ?? []) as { recommended_product_id: string }[]) {
      const id = row.recommended_product_id;
      if (seen.has(id)) continue;
      seen.add(id);
      ordered.push(id);
...
export async function getRecommendations(opts: GetRecommendationsOptions): Promise<RecResult> {
  ...
  switch (opts.surface) {
    case "pdp_similar": {
      ...
      let ids = await similarProductIds(anchor, exclude, limit);
      if (ids.length === 0) {
        // Fallback: attribute-based match on the anchor's own materials/applications
        // is unavailable without seed filters here, so fall through to popularity.
    ...
    case "pdp_cross_sell": {
      ...
      const ids = await crossSellProductIds(anchors, exclude, limit);
